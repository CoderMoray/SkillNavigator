...
【开始】检查 skillnav（--version、config test）；whoami 确认登录。未登录则引导用户到 （部署方未配置对外 Web 地址——请向平台维护者索取） 「设置→API 密钥」创建 Key，执行 skillnav login --api-key sk_…；勿向用户索要或回显完整密钥。忘记命令用 skillnav <cmd> --help。

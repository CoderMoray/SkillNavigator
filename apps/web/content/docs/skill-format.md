...
- 单文件文本审查上限约 **1 MB**；整包约 **50 MB**（服务端 `API_BODY_LIMIT_MB` 默认 50，以平台当前配置为准）。
- Web 端**选择文件夹**时由浏览器在本地打包，另有 **32 MB** 上限（提示「Skill 包总大小不能超过 32 MB」）。包较大时请自行打成 ZIP 再上传：ZIP 直传不受此限，只受上面的 50 MB 约束。
- 路径不得包含 `..` 等逃逸片段。

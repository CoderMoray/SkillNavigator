...
if node_modules/.bin/tsx "$REPO_ROOT/scripts/verify-inspection-deps.mjs"; then
  ...
  if [ "$ON_DEV" = "true" ] && [ "${REVIEW_DEPS_STRICT:-false}" != "true" ]; then
    ...
    echo "      Fix or disable providers before publishing (npm run verify:inspection-deps)."
  else
